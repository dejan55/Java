...
class Zbor implements Comparable<Zbor>{
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < zbor.length(); i++) {
            hash = ((hash << 5) + hash) + zbor.charAt(i);
        }
...
public class Speluvanje {
    public static void main(String[] args) throws IOException {
        ...
        for (int i=0; i<pom.length; i++) {
            String lowU=pom[i].toLowerCase();
            lowU=lowU.replace(".","");
            lowU=lowU.replace(",","");
            lowU=lowU.replace("!","");
            lowU=lowU.replace("?","");
            int getInd=tabela.search(new Zbor(lowU));
            if(getInd>=0){
                MapEntry<Zbor,String> tmp=tabela.getBucket(getInd);
                if(tmp!=null){
                    String pom2=tmp.toString();
                    if(!pom2.equals(lowU)) {
                        System.out.println(lowU);
                        print=false;
                    }   
        ...
        if(print) System.out.println("Bravo");
        if(tekst.charAt(0)=='.') System.out.println("Bravo");
        /*
        *
        * Vashiot kod tuka....
        *
