...
class Zbor implements Comparable<Zbor>{
	...
	@Override
	public int hashCode() {
		return zbor.hashCode() % zbor.charAt(0);
	}
	@Override
	public String toString() {
		return zbor;
...
public class Speluvanje {
	public static void main(String[] args) throws IOException {
		...
		Zbor zbor;
		for (int i = 0; i < N; i++) {
			zbor = new Zbor(br.readLine());
			tabela.insert(zbor, zbor.zbor);
		}
		String red;
		boolean ok = true;
			red = br.readLine();
			String[] zborovi = red.split(", |! |!|\\? |\\?|\\. |\\.| ");
			for (int i = 0; i < zborovi.length; i++) {
				zborovi[i] = zborovi[i].toLowerCase();
		...
		if(ok) System.out.println("Bravo");
	}
}
